...
module.exports = async function handler(req, res) {
  ...
  // Sanitize query: strip Unicode minus/dash to ASCII
  const query = typeof queryRaw === 'string'
    ? queryRaw
        .replace(/\u2212/g, '-')
        .replace(/\u2013/g, '-')
        .replace(/\u2014/g, '-')
...
async function callGemini(apiKey, prompt, maxTokens) {
  try {
    // gemini-2.0-flash: stable, no thinking tokens, reliable text output
    ...
    const r = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        generationConfig: { maxOutputTokens: maxTokens, temperature: 0.7, candidateCount: 1 },
      }),
    });
    let body;
    try { body = await r.json(); }
    catch(_) { return { error: 'Gemini returned non-JSON', status: 502 }; }
    console.log('[whatif/gemini] status:', r.status, '| finishReason:', body?.candidates?.[0]?.finishReason);
    if (!r.ok) return { error: body?.error?.message || `Gemini HTTP ${r.status}`, status: r.status };
    // CRITICAL FIX: collect ALL text parts — gemini-2.5 puts thinking in parts[0]
    const parts = body?.candidates?.[0]?.content?.parts || [];
    const text  = parts.filter(p => p.text).map(p => p.text.trim()).filter(Boolean).join('\n\n').trim();
    if (!text) {
      const reason = body?.candidates?.[0]?.finishReason;
      console.error('[whatif/gemini] empty text, finishReason:', reason);
      return { error: `Gemini empty response (${reason || 'unknown'})`, status: 502 };
    }
    return { text, model };
  } catch (err) {
